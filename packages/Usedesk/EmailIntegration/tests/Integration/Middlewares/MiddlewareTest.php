...
namespace Usedesk\EmailIntegration\Test\Integration\Middlewares;

use Illuminate\Http\Request;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\Auth;

use Usedesk\EmailIntegration\Middlewares\NameMiddleware;
...
{
    protected $NameMiddleware;

    public function setUp()
    {
        parent::setUp();
        $this->nameMiddleware = new NameMiddleware($this->app);
    }
    /** @test */
    ...
    public function a_user_can_access_a_route_protected_by_this_role_middleware_if_have_one_of_the_roles()
    {
        ...
        $this->assertEquals(
            $this->runMiddleware(
                $this->nameMiddleware, 'testRole|testRole2'
            ), 200);
        $this->assertEquals(
            $this->runMiddleware(
                $this->nameMiddleware, ['testRole2', 'testRole']
            ), 200);
    }
    /** @test */
